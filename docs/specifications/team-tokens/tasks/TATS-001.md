# team-tokens-001 — Team Access Tokens LiveView (`/t/:team_id/tokens`)
...
| ACID | Requirement |
|------|-------------|
| `team-tokens.MAIN.1` | Lists all tokens that have been created for this team, by any user |
| `team-tokens.MAIN.1-1` | Lists the token prefix and the token name, created-by attribution, other useful metadata contained in the database (see data-model feature) |
| `team-tokens.MAIN.2` | User education explains that Team Access Tokens provide full read and write access to any resources in the team, except for `tats:admin` and `teams:admin` |
| `team-tokens.MAIN.3` | Can create a new token, with name, optional expiration timestamp (date picker) |
| `team-tokens.MAIN.3-1` | Cannot edit token scopes, it's the same set of scopes for all tokens |
| `team-tokens.MAIN.4` | On creation, user is presented with the token, and educated that they will never be able to see it again |
| `team-tokens.MAIN.4-1` | New token has a copy paste button and can be cleanly highlighted with click-drag or triple-mouse click |
| `team-tokens.MAIN.5` | Tokens can be revoked by clicking a revoke button |
| `team-tokens.MAIN.5-1` | Token revocation triggers confirmation |
| `team-tokens.USAGE.1` | Renders a mock Usage section with a "coming soon" placeholder |
| `team-tokens.TATSEC.1` | The system must never persist a raw token to the database or in logs |
| `team-tokens.TATSEC.2` | API authentication requests must hash the incoming token and query against the `token_hash` column |
| `team-tokens.TATSEC.3` | A token is considered invalid if `revoked_at` is not null, or if `expires_at` is in the past |
| `team-tokens.TATSEC.4` | Respect the `tats:admin` scope for create and revoke ops |
| `team-tokens.TATSEC.5` | Respect the `team:read` scope for read ops (everyone can see the list of all existing tokens and their details) |
...
- You need to implement token generation in the context (or a dedicated private helper). The pattern:
  ...
  6. `team-tokens.TATSEC.1`: only store `token_hash` and `token_prefix`, never `raw_token`.
...
- Follow identical patterns: `stream/3` for the token list, inline modals driven by boolean assigns, `to_form/2` for all forms, ACID-only inline comments (e.g., `<%!-- team-tokens.MAIN.1 --%>`).
...
- `<.input field={@form[:expires_at]} type="datetime-local" label="Expiration (optional)" />` — date picker for expiry (`team-tokens.MAIN.3`).
- `<.header>` — page title section.
- `<.icon name="hero-*" />` — icons (e.g., `hero-key`, `hero-clipboard`, `hero-x-circle`).
- For the copy-paste button (`team-tokens.MAIN.4-1`), use a colocated JS hook (`:type={Phoenix.LiveView.ColocatedHook}`) that calls `navigator.clipboard.writeText(...)` on click, so you don't need an external hook file.
...
- [ ] **Create the LiveView template** (inline `~H` sigil):
  ...
  - Page header with team name and a "Create Token" button (disabled when `!@can_manage_tokens?`) — `team-tokens.TATSEC.4`.
  - User education callout explaining token scopes (`team-tokens.MAIN.2`): mention that tokens grant full `specs:read/write`, `refs:read/write`, `impls:read/write`, `team:read`, but do **not** include `tats:admin` or `team:admin`.
  - Token list with `phx-update="stream"` on the container:
    - For each token: display `token_prefix`, `name`, created-by user email, `inserted_at`, `expires_at` (if set), and a revoked badge when `revoked_at` is not nil (`team-tokens.MAIN.1-1`).
    - Revoke button per token row (disabled when `!@can_manage_tokens?` or token already revoked) — `team-tokens.MAIN.5`, `team-tokens.TATSEC.4`.
  - Empty state shown when `@tokens_empty?` is true.
  - Create Token inline modal:
    - Name input (`team-tokens.MAIN.3`).
    - Date-time input for optional expiration (`team-tokens.MAIN.3`). No scope selector — `team-tokens.MAIN.3-1`.
    - After successful creation, inside the modal (or replacing the form area), show the raw token in a monospace block with a copy button (colocated JS hook) and a clear warning that it will never be shown again (`team-tokens.MAIN.4`, `team-tokens.MAIN.4-1`). A "Done" button triggers `"dismiss_token"`.
  - Revoke confirmation inline modal: confirm message + Revoke / Cancel buttons (`team-tokens.MAIN.5-1`).
  - Usage section at the bottom with a "coming soon" placeholder (`team-tokens.USAGE.1`).
...
- [ ] **Write context tests** at `test/acai/teams/teams_test.exs` (add to existing file):
  ...
  - Token is treated as invalid when `revoked_at` is set or `expires_at` is in the past (`team-tokens.TATSEC.3` — verify by inspecting the returned struct after revoke / by querying a manually expired token).
...
- [ ] **Write LiveView tests** at `test/acai_web/live/team_tokens_live_test.exs`:
  ...
  - Create Token button is disabled for non-owner roles (developer, readonly) — `team-tokens.TATSEC.4`.
  - Owner can open the create modal and see the name and expiry inputs but no scopes input — `team-tokens.MAIN.3-1`.
  - Submitting the create form with a valid name inserts a new token into the stream and reveals the raw token display — `team-tokens.MAIN.4`.
  - The raw token reveal area has a copy button element — `team-tokens.MAIN.4-1`.
  - Submitting create with an empty name shows a validation error.
  - Revoke button is disabled for non-owner roles — `team-tokens.TATSEC.4`.
  - Owner clicking Revoke opens the confirmation modal — `team-tokens.MAIN.5-1`.
  - Confirming revocation marks the token as revoked in the stream (revoked badge visible).
  - The usage section with "coming soon" text is rendered — `team-tokens.USAGE.1`.
  - Use `has_element?/2` and `element/2` with DOM IDs defined in the template.

## Notes

- `team-tokens.TATSEC.1`: Never log or persist `raw_token`. The virtual field is only populated on the return value of `generate_token/3` and must not be passed to `changeset/2`.
- `team-tokens.TATSEC.2`: Hashing for API auth is a constraint to be respected in the context — the `token_hash` column is what API auth will query against. This task does not implement API auth endpoints, but the `generate_token` function must produce a correct SHA-256 hash so future auth can rely on it.
- `team-tokens.TATSEC.3`: The token-validity logic (checking `revoked_at` and `expires_at`) is a constraint that belongs in the context. Expose a `valid_token?/1` helper or inline the logic in `revoke_token` and any future auth function.
...
- ACID comments must be the identifier **only** — no appended description text (e.g., `# team-tokens.MAIN.1` not `# team-tokens.MAIN.1 — list all tokens`).
...
- **Template**: All ACID annotations present and identifier-only (the single combined `<%!-- team-tokens.MAIN.3 / team-tokens.MAIN.4 --%>` is consistent with the accepted pattern already in the codebase). `select-all` CSS class on the token `<pre>` correctly satisfies `team-tokens.MAIN.4-1` for click-drag highlighting. Colocated `.CopyToken` JS hook is correctly scoped and named.
